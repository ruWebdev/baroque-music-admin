...
async function deleteInstrument() {
    if (!confirm('Удалить этот инструмент? Это действие необратимо.')) return;
    try {
        await axios.post('/instruments/delete/' + props.data.instrument.id);
        window.location.href = '/instruments';
    } catch (e) {
        toast.warning('Не удалось удалить инструмент');
    }
}
...
<template>
    ...
    <ContentLayout>
        ...
        <template #RightButtons>
            <button class="btn btn-outline-danger me-2 d-none d-sm-inline-block" @click="deleteInstrument()">
