...
<body class="min-h-screen bg-slate-950 text-slate-50 flex items-center justify-center">
    <div class="w-full max-w-lg px-6 py-8">
        ...
        <p class="text-xs text-slate-400 mb-6 text-center">Укажите основную информацию о событии</p>
        ...
        <form action="{{ url('/telegram-mini-app/event') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div class="space-y-1">
                <label class="block text-xs font-medium text-slate-200">Название</label>
                <input type="text" name="title" value="{{ old('title') }}"
                    class="w-full rounded-xl bg-slate-900/70 border border-slate-700 px-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500">
            </div>

            <div class="space-y-1">
                <label class="block text-xs font-medium text-slate-200">Описание</label>
            ...
            <div class="space-y-1">
                ...
                <textarea name="performers" rows="2"
                    class="w-full rounded-xl bg-slate-900/70 border border-slate-700 px-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" placeholder="Например: ансамбль, солисты и т.п.">{{ old('performers') }}</textarea>
            </div>

            <div class="space-y-1">
                <label class="block text-xs font-medium text-slate-200">Программа</label>
                <textarea name="program" rows="3"
                    class="w-full rounded-xl bg-slate-900/70 border border-slate-700 px-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" placeholder="Кратко опишите программу концерта или события">{{ old('program') }}</textarea>
            </div>
